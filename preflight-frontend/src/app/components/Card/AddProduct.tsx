...
const AddProduct = ({ onProductAdded }: AddProductProps) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!Number.isFinite(formData.amount) || formData.amount < 0) e.amount = 'Amount must be 0 or greater';
        setErrors(e);
    ...
    const handleAddProduct = async () => {
        ...
        try {
            ...
            alert(`Product "${result.data?.title ?? formData.title}" added successfully!`);
        } catch (error) {
            console.error(error);
        ...
        <>
            ...
            <AddPopupStyled $showPopup={isShowPopup}>
                <div className="flex flex-col gap-4 p-6 w-full bg-white rounded-xl shadow-md">
                    <h2 className="text-xl font-semibold text-gray-800 mb-2">Add Product</h2>
                    ...
                    <TextField
                        ...
                        error={!!errors.amount}
                        helperText={errors.amount || 'Required field - must be 0 or greater'}
                        disabled={isLoading}
                        required
